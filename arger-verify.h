...
namespace arger::detail {
	...
	struct ValidArguments {
		const detail::_Arguments* args = 0;
	...
	struct ValidConfig : public detail::ValidArguments {
		...
		const arger::_Config* config = 0;
	};
	struct ValidGroup : public detail::ValidArguments {
		const arger::_Group* group = 0;
	...
	inline void ValidateConfig(const arger::_Config& config, detail::ValidConfig& state) {
